...
class ConnectionButton extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // اگر هیچ کانفیگی انتخاب نشده باشد، دکمه غیرفعال می‌شود
    // If no config is selected, the button is disabled
    final bool noConfigSelected = selectedConfig == null;
    ...
    return _ConnectionButton(
      onTap: switch (connectionStatus) {
        AsyncData(value: Disconnected()) || AsyncError() =>
          noConfigSelected
              ? () {
                  CustomAlertDialog(title: "هشدار", message: "لطفا ابتدا یک کانفیگ انتخاب کنید").show(context);
                }
              : () async {
        ...
        AsyncData(value: Connected()) => () async {
          if (requiresReconnect == true && await showExperimentalNotice()) {
            return await ref.read(connectionNotifierProvider.notifier).reconnect(selectedConfig);
          }
          return await ref.read(connectionNotifierProvider.notifier).toggleConnection();
        },
        _ => () {},
      ...
      enabled: switch (connectionStatus) {
        ...
        AsyncData(value: Disconnected()) => !noConfigSelected,
        AsyncError() => !noConfigSelected,
        _ => false,
      },
      ...
      buttonColor: switch (connectionStatus) {
        ...
        AsyncData(value: _) => noConfigSelected ? Colors.grey : buttonTheme.idleColor!,
        _ => Colors.red,
      },
